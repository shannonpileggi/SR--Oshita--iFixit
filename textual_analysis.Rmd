...
##### Average and median first_punc grouped by answered

```{r, echo = FALSE}
x %>%
  group_by(answered) %>%
  summarise(avg_pos = mean(first_punct, na.rm = TRUE), median_pos = median(first_punct, na.rm = TRUE), n = n())
```

##### Average and median first_punct grouped by category
...
##### Median time_until_answer grouped by whether or not text contains punct
...
x %>%
  group_by(contain_punct) %>%
  summarise(median_time = median(time_until_answer), n = n())
```

##### Proportion of answered/unanswered questions that contain punct

```{r, echo = FALSE}
x %>%
  group_by(answered) %>%
  summarise(n = n(), prop_contain = sum(contain_punct)/n)
```

##### Proportion that contain punct grouped by categories
...
##### Median time_until_answer grouped by whether or not text ends with punct
...
##### Proportion of answered/unanswered questions that end with punct
...
x$contain_question <- str_detect(x$title, pattern = QUESTION)
...
##### median time grouped by whether or not title contains: '?'
...
##### median time and proportion answered grouped by begin_wh
```{r, echo = FALSE}
x %>%
  group_by(begin_wh) %>%
  summarise(median_time = median(time_until_answer), n = n(), prop_answered = sum(answered)/n)
```
...
x %>%
  group_by(answered) %>%
  summarise(n = n(), prop_capital = sum(capital_text)/n)
```
...
### Checking if text is in single caps (all lower or upper case)
...
##### Proportion all lower case grouped by answered/unanswered 

```{r, echo = FALSE}
x %>%
  group_by(answered) %>%
  summarise(n = n(), prop_allLower = sum(all_lower)/n)
```
...
```{r, echo = FALSE}
x %>%
  group_by(prior_effort) %>%
  summarise(median_time = median(time_until_answer), avg_daily = mean(daily_views), n = n()) %>%
  arrange(median_time)
```

##### proportion within answered/unanswered questions that indicate prior effort

```{r, echo = FALSE}
x %>%
  group_by(answered) %>%
  summarise(n = n(), prop_indicateeffort = sum(prior_effort)/n) 
```
...
##### Median time grouped by whether or not gratitude is expressed
...
##### proportion in answered/unanswered questions that express gratitude
...
### Does greeting people matter? (presence of "hi")
