...
* **minimum:** `r round(min(x$time_until_answer), 2)` hrs
* **maximum:** `r round(max(x$time_until_answer), 2)` hrs, `r round(max(x$time_until_answer)/24, 2)` days
...
* Within an hour, `r round((sum(x$time_until_answer <= 1)/nrow(x))*100,2)`% of the questions had been answered
* Within 5 hours, `r round((sum(x$time_until_answer <= 5)/nrow(x))*100,2)`% of the questions had been answered
...
* **rmean survival time:** `r round(s$table[5], 2)` hrs, `r round(s$table[5]/24, 2)` days
...
* **Interpretations:**
    ...
    + highest quantile that exists: `r round(quant$quantile[6], 2)` hrs 64% of the questions have been answered 
...
ggsurvplot(KM, data = x, risk.table = TRUE, xlab = "Time (hours)", main = "KM Curve")
ggsurvplot(KM, data = x, risk.table = FALSE, xlab = "Time (hours)", main = "KM Curve (zoomed in)", xlim = c(0,50), break.x.by = 10)
```

##### Reversing the survival curves

* at 100 hrs, the probability that a question is answered starts to level out at around ~0.63
...
ggplot(reversed, aes(x = time, y = prob_ans)) + 
  ...
  scale_y_continuous("Probability a question is answered") + 
  ggtitle("Probability a question is answered vs. time (hours)")
```
