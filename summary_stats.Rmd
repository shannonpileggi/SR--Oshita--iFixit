...
* Survival curve shows that survival probability drops immediately after question is posted. 
* Within a few hours after posting, the survival probability levels out (though it still decreases at a slow rate), and hovers around 0.35 (never hits 0).

```{r, survival curves, echo = FALSE}
ggsurvplot(KM, data = x, risk.table = TRUE, xlab = "Time (hours)", title = "KM Curve")
```


```{r, echo = FALSE}
ggsurvplot(KM, data = x, risk.table = FALSE, xlab = "Time (hours)", title = "KM Curve (zoomed in)", xlim = c(0,50), break.x.by = 10)
```
...
* Max instant hazard rate: `r round(max(h.hat$est), 2)` at `r round(h.hat$time[which(h.hat$est == max(h.hat$est))], 2)` hrs 
