...
ggplot(top50, aes(x = word, y = frequency/nrow(out_english))) + 
  geom_point() +
...
prop_contain <- sum(out_english$contain_top10)/nrow(out_english)
```
...
```{r, echo = FALSE}
out_english$contain_u <- str_detect(as.character(out_english$title), pattern = or1(freq_terms_u$word))

prop_contain_u <- sum(out_english$contain_u)/nrow(out_english)
```

* `r round(prop_contain_u * 100, 2)`% of questions contain at least one of the most frequent unanswered terms

##### median answer time and average daily views grouped by whether or not question contains at least 1 uanswered term 
...
plot_surv(KM_u, data = out_english)
```
...
```{r, echo = FALSE}
out_english$contain_a <- str_detect(as.character(out_english$title), pattern = or1(freq_terms_a$word))

prop_contain_a <- sum(out_english$contain_a)/nrow(out_english)
```

* `r round(prop_contain_a * 100, 2)`% of questions contain at least one of the most frequent answered terms

##### median answer time and average daily views grouped by whether or not question contains at least 1 answered term 
...
```{r, echo = FALSE}
survdiff(surv_object~contain_a, data = out_english)
```

```{r, echo = FALSE}
KM_a <- survfit(surv_object ~ contain_a, data = out_english)
plot_surv(KM_a, data = out_english)
```
...
out_english$contain_u_dev <- str_detect(as.character(out_english$title), pattern = or1(unanswered_devices$word))

prop_contain_u_dev <- sum(out_english$contain_u_dev)/nrow(out_english)
```
...
plot_surv(KM_u_dev, data = out_english)
```
...
out_english$contain_dev_a <- str_detect(as.character(out_english$title), pattern = or1(answered_devices$word))
prop_contain_a_dev <- sum(out_english$contain_dev_a)/nrow(out_english)
```
...
plot_surv(KM_dev_a, data = out_english)
```