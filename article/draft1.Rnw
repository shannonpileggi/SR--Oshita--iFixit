...
\end{figure}
  ...
  Of 8,025 questions in the full data, 7,760 were in English (97\% of the full data). Of those questions, \Sexpr{format(answered, big.mark = ",")} (\Sexpr{percentans}}\%) received an answer by the download date. The shortest response time was \Sexpr{shortest} hours. The longest was \Sexpr{format(longest, big.mark = ",")} hours (\Sexpr{longest_d} days). Figure \ref{fig:answertimes} shows the distribution of response times for all questions analyzed. 
...
\begin{tabular}{|l L{5.5cm}|c|c|}
 ...
 \textbf{Variable} & & \textbf{Coefficient (SE)} & \textbf{p-value} \\ \hline
  ...
  \multicolumn{2}{|L{9cm}|}{Title contains terms considered frequently-used among unanswered questions} & -0.2808 (0.0340)  & <0.0001 \\ \hline
  \multicolumn{2}{|L{9cm}|}{Title ends in a question mark} & 0.2571 (0.0332) &  <0.0001 \\ \hline
