...
# creating data frame with model coefficients + HR + 95% CI
...
# extracting SE from variance matrix
model_coefs$SE <- 0
for (i in 1:32) {
...
# rounding 
model_coefs$beta <- round(model_coefs$beta, 2)
model_coefs$HR <- round(model_coefs$HR, 2)
model_coefs$SE <- round(model_coefs$SE, 3)
model_coefs$upper <- round(model_coefs$upper, 2)
model_coefs$lower <- round(model_coefs$lower, 2)

# removing continuous predictors with splines 
model_coefs <- model_coefs[-which(stringr::str_detect(model_coefs$vars, 
                                                      pattern = rebus::or(START %R% "text_length",
                                                                          START %R% "avg_tag_length",
                                                                          START %R% "device_length",
                                                                          START %R% "newline_ratio"))), ]

# to include in abstract
# HR and confidence intervals
a <- which(stringr::str_detect(model_coefs$vars, pattern = "Apple Product"))
appleHR <- model_coefs$HR[a]
appleUpper <- model_coefs$upper[a]
appleLower <- model_coefs$lower[a]

t <- which(stringr::str_detect(model_coefs$vars, pattern = "questionmark"))
titleHR <- model_coefs$HR[t]
titleUpper <- model_coefs$upper[t]
titleLower <- model_coefs$lower[t]

w <- which(stringr::str_detect(model_coefs$vars, pattern = "Weekend"))
dayHR <- model_coefs$HR[w]
dayUpper <- model_coefs$upper[w]
dayLower <- model_coefs$lower[w]

# model R2
r2 <- round(model$stats[which(names(model$stats) == "R2")], 2)
@
...
\begin{abstract} 
Community-driven online question and answer forums (CQA), platforms that allow users to ask and answer questions, contain an extensive amount of crowd-sourced information. One such example of a CQA is iFixit's \textit{Answers} forum. This forum features user-asked questions related specifically to device repair, which are answered by both repair experts and everyday users. An important aspect of these CQAs, for both users and the forum, is question response time. This paper presents a survival analysis of the time until a question posted on iFixit's \textit{Answers} forum receives it's first response. A Cox proportional hazards model was developed to identify variables associated with response time. Though several predictors were significant, the model's predictive accuracy was low ($R^2 = \Sexpr{r2}$). Significant predictors included the device category of the question (questions pertaining to Apple products received answers faster than others (HR = \Sexpr{appleHR}, 95\% CI = (\Sexpr{appleLower}, \Sexpr{appleUpper})), factors related to the question's title (e.g., if it was phrased as a question (HR = \Sexpr{titleHR}, 95\% CI = (\Sexpr{titleLower}, \Sexpr{titleUpper}))), and the day of the week the question was posted (questions posted over the weekend received answers slower than those posted on a weekday (HR = \Sexpr{dayHR}, 95\% CI = (\Sexpr{dayLower}, \Sexpr{dayUpper}))). Future studies can investigate whether factors identified as significant in this analysis can be generalized to other CQAs.
\end{abstract}
...
% Unless we are planning to make the oshitar package available on cran, it might be better to setup a GIST with a clean version of the data and the variable_setup() function.  Or we should at least link to the github repo instead: https://github.com/loshita/oshitar
...
longest <- round(max(x$time_until_answer))
longest_d <- round(longest / 24, 1) 
@

% Distribution of answer times
\begin{figure}[!htbp]
  \includegraphics[scale=0.5]{times_dist.pdf}
...
% Kaplan-Meier curve
\begin{figure}[!htbp]
  \includegraphics[scale=0.5]{kmcurve.pdf}
  \label{fig:kmcurve}
\end{figure}
% \caption{Kaplan-Meier curve for all questions}

% Heyyy this is a dynamic document, why not make these images in-line? :-()

  Of 8,025 questions in the full data, \Sexpr{scales::comma(nrow(x))} were in English (97\% of the full data). Of those questions, \Sexpr{scales::comma(answered)} (\Sexpr{percentans}\%) received an answer by the download date. The shortest response time was \Sexpr{shortest} hours. The longest was \Sexpr{scales::comma(longest)} hours (\Sexpr{scales::comma(longest_d)} days). Figure \ref{fig:answertimes} shows the distribution of response times for all questions analyzed. 

% fyi you can use scales::comma(val) to format numbers too
...
% cool, I like this table!
% --> can we write the Predictor column more mathematically?
% --> e.g., $\sqrt{\textnormal{average tag frequency}}$
% --> e.g., $\log\left({\textnormal{device length} + 1}\right)$
% also, the 'K' column might align better if we put the * to the right
...
% This table looks nice, but the code is brutal and not reproducible.  I feel compelled to point out 'xtable' or 'stargazer' for auto-generated latex tables like this using R.

% I think we should use 2 digits for coefficients and 3 digits for SE/p-value terms

<< echo = FALSE >>=
LR <- unname(round(model$stats[which(names(model$stats) == "Model L.R.")], 2))
dxy <- unname(round(model$stats[which(names(model$stats) == "Dxy")], 2))
@
...
As a whole, the final model was significant with a partial likelihood ratio of \Sexpr{LR} (p-value \textless0.0001). Its $R^2$ statistic was \Sexpr{r2}, and Somers' \textit{Dxy} was \Sexpr{dxy}. 
...
<< echo = FALSE >>=
apple_p <- (appleHR - 1) * 100
apple_p_l <- (appleLower - 1) * 100
apple_p_u <- (appleUpper - 1) * 100

day_p <- (1 - dayHR) * 100
day_p_u <- (1 - dayLower) * 100
day_p_l <- (1 - dayUpper) * 100

u <- which(stringr::str_detect(model_coefs$vars, pattern = "contain_unanswered"))
unansw_p <- (1 - model_coefs$HR[u]) * 100
unansw_p_l <- (1 - model_coefs$upper[u]) * 100
unansw_p_u <- (1 - model_coefs$lower[u]) * 100
@


\begin{itemize}
  \item The estimated hazard of receiving an answer is \Sexpr{apple_p}\% higher (95\% CI (\Sexpr{apple_p_l}\%, \Sexpr{apple_p_u}\%)) for questions pertaining to Apple products than the hazard for questions about Android and Other phones, controlling for all other predictors.
  \item The estimated hazard of receiving an answer is \Sexpr{unansw_p}\% lower (95\% CI (\Sexpr{unansw_p_l}\%, \Sexpr{unansw_p_u}\%)) for questions with titles that contain at least one word considered to be frequently-used among unanswered questions, than the hazard for questions with titles that do not, controlling for all other predictors. 
  \item The estimated hazard of receiving an answer for questions posted on a weekend is \Sexpr{day_p}\% lower (95\% CI (\Sexpr{day_p_l}\%, \Sexpr{day_p_u}\%)) than the hazard for questions posted on a weekday, controlling for all other predictors.
\end{itemize}
...
% If there ever was a good time to use a wordlcloud, I think this is it!! Just sayin...
% library(wordcloud)
...
% \bibliogrpahystyle{elsarticle-harv}
