...
\begin{abstract} 
iFixit's community-driven online question and answer forum (CQA), \textit{Answers}, provides a platform for thousands of users to ask and answer device repair-related questions. This paper presents a survival analysis of the time until a question posted on iFixit's \textit{Answers} forum receives it's first response. A Cox proportional hazards model was developed to identify variables associated with response time. Though several predictors were significant, the model's predictive accuracy was low ($R^2 = \Sexpr{r2}$). Significant predictors included the device category of the question (questions pertaining to Apple products received answers faster than others (HR = \Sexpr{appleHR}, 95\% CI = (\Sexpr{appleLower}, \Sexpr{appleUpper})), factors related to the question's title (e.g., if it was phrased as a question (HR = \Sexpr{titleHR}, 95\% CI = (\Sexpr{titleLower}, \Sexpr{titleUpper}))), and the day of the week the question was posted (questions posted over the weekend received answers slower than those posted on a weekday (HR = \Sexpr{dayHR}, 95\% CI = (\Sexpr{dayLower}, \Sexpr{dayUpper}))). Future studies can investigate whether factors identified as significant in this analysis can be generalized to other CQAs.
