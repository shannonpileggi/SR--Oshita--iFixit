...
\begin{abstract} 
iFixit's community-driven online question and answer forum (CQA), \textit{Answers}, contains an extensive amount of crowd-sourced information related specifically to device repair. This platform allows users to ask questions regarding repairs, and for both repair experts and everyday users to provide answers and feedback. This paper presents a survival analysis of the time until a question posted on iFixit's \textit{Answers} forum receives it's first response. A Cox proportional hazards model was developed to identify variables associated with response time. Though several predictors were significant, the model's predictive accuracy was low ($R^2 = \Sexpr{r2}$). Significant predictors included the device category of the question (questions pertaining to Apple products received answers faster than others (HR = \Sexpr{appleHR}, 95\% CI = (\Sexpr{appleLower}, \Sexpr{appleUpper})), factors related to the question's title (e.g., if it was phrased as a question (HR = \Sexpr{titleHR}, 95\% CI = (\Sexpr{titleLower}, \Sexpr{titleUpper}))), and the day of the week the question was posted (questions posted over the weekend received answers slower than those posted on a weekday (HR = \Sexpr{dayHR}, 95\% CI = (\Sexpr{dayLower}, \Sexpr{dayUpper}))). Future studies can investigate whether factors identified as significant in this analysis can be generalized to other CQAs.
\end{abstract}
...
% \end{figure}
  ...
  Of 8,025 questions in the full data, \Sexpr{scales::comma(nrow(x))} were in English (97\% of the full data). Of those questions, \Sexpr{scales::comma(answered)} (\Sexpr{percentans}\%) received an answer by the download date. The shortest response time was \Sexpr{shortest} hours. The longest was \Sexpr{scales::comma(longest)} hours (\Sexpr{scales::comma(longest_d)} days). Figure 1 shows the distribution of response times for all questions analyzed. 
...
This variable indicates true if the asker included words in the question's text that indicate that prior effort was made to find the solution to the question before posting. Words searched for include: tried, searched, researched, tested, replaced, used, checked, investigated, considered, measured, attempted, inspected, fitted. This variable was created based off of findings and methods from \cite{Bhat2014} and \cite{Harper2008}. 
