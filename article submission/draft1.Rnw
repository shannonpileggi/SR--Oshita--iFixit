...
\end{figure}
  ...
  Of \Sexpr{nrow(x)} questions, \Sexpr{answered} (\Sexpr{percentans}}\%) received an answer by the download date. The shortest answer time was \Sexpr{shortest} hours. The longest was \Sexpr{longest} hours (\Sexpr{longest_d} days). Figure ~\ref{fig:answertimes} shows the distribution of answer times for all questions, grouped by whether or not the question received an answer. 
