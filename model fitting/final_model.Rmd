...
x <- read.csv("final_data.csv", head = TRUE)
...
final <- rms::cph(Surv(time_until_answer, answered) ~ new_category + new_user + contain_unanswered + contain_answered + title_questionmark + title_beginwh + text_contain_punct + text_all_lower + update + greeting + gratitude + prior_effort + weekday + strat(ampm) + sqrt(avg_tag_score) + pol(text_length, 2) + rcs(device_length, 5) + rcs(avg_tag_length, 4) + rcs(newline_ratio, 4), data = x, x = TRUE, y = TRUE, surv = TRUE, time.inc = 0.5, dxy = TRUE)

validation <- rms::validate(final, method = "crossvalidation", B = 100, bw = F, dxy = TRUE, u = 0.5)
validation
```
...
```{r}
# plots log hazard for each variable 
plots <- ggplot(Predict(final)) # output is messy, figure out how to access each plot separately 

times <- sort(unique(x$time_until_answer))
pec::plotPredictSurvProb(final, newdata = x[1:4,], times = times) # ugly output

# must specify a variable 
rms::survplot(final, ampm, time.inc = 100, col = 1:4, n.risk = TRUE)

# plot the effects of predictors
plot(Predict(final, new_category))
plot(Predict(final, new_user))
plot(Predict(final, new_user, new_category))
...
# function runs really slow
# outputs matrix of predicted survival probabilities 
# sets up variables needed in the new data 
get_survival <- function(times, data) {
  # setting up necessary variables in new data 
  newdata <- oshitar::variable_setup(data)
  
  # imports data to build model on 
  x <- read.csv("final_data.csv", head = TRUE)
  # builds model 
  ...
  # gets survival probabilities 
  surv_probs <- pec::predictSurvProb(model, newdata = newdata, times)
  return(surv_probs)
}

get_survival(times = c(0.5, 1, 10), data = out[1:10,])
```
