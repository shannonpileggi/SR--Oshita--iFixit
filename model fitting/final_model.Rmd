...
```{r, echo = FALSE}
# validation and calibration with rms package

valmodel <- rms::cph(Surv(time_until_answer, answered) ~ new_category + new_user + contain_unanswered + contain_answered + title_questionmark + title_beginwh + text_contain_punct + text_all_lower + update + greeting + gratitude + prior_effort + weekday + strat(ampm) + sqrt(avg_tag_score) + poly(text_length, 2) + rcs(device_length, 5) + rcs(avg_tag_length, 4) + rcs(newline_ratio, 4), data = x, x = TRUE, y = TRUE, surv = TRUE, time.inc = 0.5, dxy = TRUE)

validation <- rms::validate(calmodel, method = "crossvalidation", B = 100, bw = F, dxy = TRUE, u = 0.5)
validation
# output: 
...
# validate the model for calibration accuracy in predicting probability at 0.5 hours 
calibration <- rms::calibrate(calmodel, method = "crossvalidation", B = 100, bw = FALSE, u = 0.5)
calibration
plot(calibration)
# predicts and calibrates at 0.5 hours for each iteration of the bootstrap
# plot gives mean absolute error in predictions, mean squared error, and the 0.9 quantile of the absolute error
...
```{r, echo = FALSE}
# getting estimated survival probabilites
...
# gives one survival probability (not split up into strat levels)
