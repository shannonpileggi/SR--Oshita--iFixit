...
##### Trying out different variable selection functions
...
train1$mart <- residuals(final, type = "martingale")
plot_mart <- function(var) {
  ggplot(train1, aes(x = train1[[var]], y = mart)) + 
    geom_point() + 
    geom_smooth() + 
...
q_vars <- c("n_images", "device_length", "newline_ratio")
purrr::map(q_vars, plot_mart)

#=============================================
#deviance residuals 
train1$deviance <- residuals(final, type = "deviance")
...
schoen <- as.data.frame(residuals(final, type = "schoenfeld"))
...
#=============================================
# score residuals
score <- as.data.frame(residuals(final, type = "score"))
...
test_predict <- predict(final, newdata = test1, type = "risk", se.fit = TRUE)
