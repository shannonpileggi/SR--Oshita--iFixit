...
# results of second stepAIC
results2 <- coxph(Surv(time_until_answer, answered) ~ new_category + as.factor(new_user) + 
...
AIC(model_spline5, k = 2) # 64841.25
```
...
list <- list(allsplines, splinedev, splinetext, splinetag, splinenew, nospline, devtext, sqrtdev, sqrttext, lots, lots2, devtag)
...
# variables : new_category + as.factor(new_user) + 
#     contain_unanswered + title_questionmark + sqrt(avg_tag_score) + 
#     text_all_lower + update + prior_effort + ampm + weekday + 
#     pspline(device_length, df = 0, method = "AIC") + pspline(avg_tag_length, 
#     df = 0, method = "AIC") 
#=============================================
#martingale residuals
train1$mart <- residuals(final_model, type = "martingale")
...
#=============================================
#deviance residuals 
train1$deviance <- residuals(final_model, type = "deviance")
...
#=============================================
# schoenfeld residuals
schoen <- as.data.frame(residuals(final_model, type = "schoenfeld"))
...
options(scipen=999)
ph_test <- data.frame(predictors = rownames(cox.zph(final_model)$table), cox.zph(final_model)$table) 
...
#=============================================
# score residuals
score <- as.data.frame(residuals(final_model, type = "score"))
