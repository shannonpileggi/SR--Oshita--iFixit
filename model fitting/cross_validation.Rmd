...
full <- coxph(Surv(time_until_answer, answered) ~ strata(new_category) + new_user + contain_unanswered + contain_answered + title_questionmark + title_beginwh + text_contain_punct + text_all_lower + update + greeting + n_tags + gratitude + prior_effort + strata(ampm) + weekday + n_images + sqrt(avg_tag_score) + pspline(device_length, df = 5) + poly(text_length, 2) + pspline(avg_tag_length, 3) + pspline(newline_ratio, 3), data = train1)
...
outliers <- train1[abs(train1$deviance) > 2.5,]
...
options(scipen=999)
ph_test <- data.frame(predictors = rownames(cox.zph(full)$table), cox.zph(full)$table) 
