...
model_spline3 <- coxph(Surv(time_until_answer, answered) ~ new_category + as.factor(new_user) + contain_unanswered + contain_answered + title_questionmark + sqrt(avg_tag_score) + text_all_lower + update + prior_effort + ampm + weekday + pspline(newline_ratio, df = 0, method = "AIC"), data = train1)
summary(model_spline3)
AIC(model_spline3, k = 2) # 64915.6
```
...
```{r}
allspline_avg <- get_avgmetrics(allsplines)
splinedev_avg <- get_avgmetrics(splinedev)
splinetext_avg <- get_avgmetrics(splinetext)
splinetag_avg <- get_avgmetrics(splinetag)
splinenew_avg <- get_avgmetrics(splinenew)
nospline_avg <- get_avgmetrics(nospline)
devtext_avg <- get_avgmetrics(devtext)
sqrtdev_avg <- get_avgmetrics(sqrtdev)
sqrttext_avg <- get_avgmetrics(sqrttext)

allspline_avg; splinedev_avg; splinetext_avg; splinetag_avg; splinenew_avg; nospline_avg; devtext_avg; sqrtdev_avg; sqrttext_avg
```
...
list <- list(allsplines, splinedev, splinetext, splinetag, splinenew, nospline, devtext, sqrtdev, sqrttext)
names <- c("allsplines", "splinedev", "splinetext", "splinetag", "splinenew", "nospline", "devtext", "sqrtdev", "sqrttext") # change this to avoid hard coding 

l_diff <- purrr::map(list, ~get_avgdiff(.))
df_diff <- plyr::ldply(purrr::map(1:length(l_diff), ~data.frame(as.list(l_diff[[.]]))), data.frame)
rownames(df_diff) <- names
df_diff
