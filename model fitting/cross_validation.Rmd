...
r2_i <- coxph(Surv(time_until_answer, answered) ~ category + n_images + new_user + weekday + ampm + device_length + title_questionmark + title_beginwh + capital_text + update + newline_ratio + frequent_tag + contain_answered + contain_unanswered + title_questionmark*title_beginwh + new_user*update + new_user*newline_ratio, data = train1)
...
# if there are less than 60 questions in a category, group them with other
counts <- train1 %>%
...
train1$mart <- residuals(r2_new, type = "martingale")

plot_res <- function(var, residuals, data, title) {
  ggplot(data, aes(x = data[[var]], y = residuals)) +
    geom_point() +
    geom_smooth() + 
    ggtitle(title) + 
...
res_image <- plot_res(var = "n_images", residuals = mart, data = train1, title = "Martingale Residuals for n_images")
res_dev <- plot_res("device_length", mart, data = train1, title = "Martingale Residuals for device_length")
res_newline <- plot_res("newline_ratio", mart, data = train1, title = "Martingale Residuals for newline_ratio")

gridExtra::grid.arrange(res_image, res_dev, res_newline, nrow=1, ncol=3)

#=============================================
#deviance residuals 
train1$deviance <- residuals(r2_new, type = "deviance")
...
schoen <- as.data.frame(residuals(r2_new, type = "schoenfeld"))
...
names <- names(schoen)[-31]
purrr::map(names, plot_schoen)

#=============================================
# score residuals
score <- as.data.frame(residuals(r2_new, type = "score"))
