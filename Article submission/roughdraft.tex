...
\section{Results} 
  ...
  Of 7,760 questions, 63.8\% received an answer by the download date. Figure a shows the distribution of answer times for all questions, grouped by whether or not the question received an answer. The following statistics were calculated from Kaplan-Meier estimates of survival probability. The mean survival time, or the average time until a question received its first answer was 775.75 hours, or 32.32 days. The median survival time, the time at which 50\% of the questions in the data received an answer was 9.16 hours. Table 1 provides more percentiles of survival time. Figure B shows the Kaplan-Meier curve for all questions in the data. The curve indicates that at around 100 hours after a question is posted, survival probability remains constant at around 0.35.

Predictors with p-values less than 0.01 in univariate analysis are in table b, and were included in the full Cox model for cross-validation. Average metrics for test and training sets are in table c. Although not impressively high, metrics did not change substantially from training to test set, indicating that the model does not overfit. Restricted cubic splines were fit to continous predictors. Splines of five knots were fit to the average frequency of question's tags as well as to the number of characters in the user-defined device name. Splines of four knots were fit to the number of characters in a question's text, average number of characters in a question's tags, and the ratio of number of line breaks to the number of characters in a question's text. Results of cross-validation for the model including the splines are in table d. As refitting the model without insignificant predictors did not effect predictive performance, all variables were included. The final model, with all variables described above including splines, was fit to the full data.  
