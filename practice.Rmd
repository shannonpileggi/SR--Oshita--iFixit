...
#26 observations in out dataframe have negative time to event variable
length(which(out$time_until_answer < 0))
library(dplyr)
out %>% 
  filter(time_until_answer < 0) %>%
  select(post_date, first_answer_date, answered)
```
...
#view average time until answered grouped by whether or not question contains "?", for each category
