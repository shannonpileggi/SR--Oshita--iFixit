...
boxplot(out_english$time_until_answer)
bstats <- boxplot.stats(out_english$time_until_answer)
bstats
...
out_english$title_length_intervals <- cut(out_english$title_length, breaks = 8)
...
### exploring device length (find another way to classify how detailed device is)
...
out_english %>%
  group_by(category, as.factor(n_tags)) %>%
...
arranged <- out1_positive %>% arrange(time_until_answer)
fastest <- arranged[1:2000,]

library(qdap)
library(tm)
# INSTALLING RWEKA NOT WORKING
# library(RWeka)
# 
